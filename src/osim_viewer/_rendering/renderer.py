...
from osim_viewer._rendering.scene.camera import CameraInterface, ViewerCamera
from osim_viewer._rendering.scene.node import Node
from osim_viewer._rendering.scene.scene import Scene
from osim_viewer._rendering.shaders import load_program
...
class Renderer:
    def __init__(self, ctx: moderngl.Context, wnd: BaseWindow, window_type: str):
        ...
        self.frag_pick_prog["position_texture"].value = 0
        self.frag_pick_prog["obj_info_texture"].value = 1
        self.picker_output = self.ctx.buffer(reserve=6 * 4)
    ...
    def render_viewport(
        self, scene: Scene, camera: CameraInterface, viewport: Viewport, **kwargs
    ):
        ...
        if not export:
            self.render_outline(
                [l for l in scene.lights if l.enabled],
                light_outline_color,
                camera,
                viewport,
            )
            if isinstance(scene.selected_object, Node):
                self.render_outline(
                    [scene.selected_object], selected_outline_color, camera, viewport
                )
    ...
    def get_current_mask_image(
        ...
        img = Image.frombytes(
            "RGBA", (ids.shape[1], ids.shape[0]), output.tobytes()
        ).convert("RGB")
