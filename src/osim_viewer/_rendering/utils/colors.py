...
def skining_weights_to_color(skinning_weights, alpha):
    ...
    colors = vertex_colors_from_weights(
        joints_ids, scale_to_range_1=True, alpha=alpha, shuffle=True, seed=1
    )
    weights_color = np.matmul(skinning_weights, colors)
    return weights_color


def vertex_colors_from_weights(
    weights, scale_to_range_1=True, alpha=None, shuffle=False, seed=0
):
    """
