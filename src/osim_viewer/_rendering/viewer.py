...
from osim_viewer._rendering.renderables.meshes import Meshes, VariableTopologyMeshes
from osim_viewer._rendering.renderables.point_clouds import PointClouds
from osim_viewer._rendering.renderer import Renderer, Viewport
from osim_viewer._rendering.scene.camera import PinholeCamera, ViewerCamera
from osim_viewer._rendering.scene.node import Node
from osim_viewer._rendering.scene.scene import Scene
from osim_viewer._rendering.shaders import clear_shader_cache
from osim_viewer._rendering.utils import path
from osim_viewer._rendering.utils.imgui_integration import ImGuiRenderer
from osim_viewer._rendering.utils.perf_timer import PerfTimer
...
class Viewer(moderngl_window.WindowConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Accept the former default in existing user configuration without Qt.
        if self.window_type == "pyqt5":
            self.window_type = "pyglet"
        ...
        self.window_size = self.window.buffer_size
        ...
        self._middle_mouse_button = 3
        self._right_mouse_button = 2
    ...
    def set_ortho_grid_viewports(self, invert=(False, False, False)):
        ...
        axis = [np.array([1, 0, 0]), np.array([0, 0, 1]), np.array([0, 1, 0])]
        for i in range(3):
            if invert[i]:
                axis[i] = -axis[i]
        up = [np.array([0, 1, 0]), np.array([0, 1, 0]), np.array([0, 0, 1])]
    ...
    def viewport_mode(self, mode):
        ...
        if mode == "single":
            new_num_viewports = 1
        elif mode == "split_v" or mode == "split_h":
            new_num_viewports = 2
        elif mode == "split_vh":
            new_num_viewports = 4
        else:
        ...
        if num_viewports > new_num_viewports:
            for v in self.viewports[new_num_viewports:]:
                if self.scene.selected_object == v.camera and isinstance(
                    v.camera, ViewerCamera
                ):
    ...
    def lock_to_node(
        self, node: Node, relative_position, smooth_sigma=None, viewport=None
    ):
        """
        ...
        from its target. See osim_viewer._rendering.utils.path.lock_to_node for more details about parameters.
    ...
    def _refresh_ui_fonts(self):
        """Rasterize fonts at framebuffer density while laying out in UI units."""
        self.imgui.resize(*self.wnd.buffer_size)
        density = max(self.imgui.io.display_fb_scale)
        if self._font_density == density:
            return
        self.fonts.clear()
        self.fonts.add_font_from_file_ttf(
            os.path.join(self.font_dir, "DejaVuSans.ttf"), 17 * density
        )
        self.custom_font = self.fonts.add_font_from_file_ttf(
            os.path.join(self.font_dir, "Custom.ttf"), 17 * density
        )
        self.imgui.io.font_global_scale = 1 / density
        self.imgui.refresh_font_texture()
        self._font_density = density
    ...
    def gui(self):
        self._refresh_ui_fonts()
        imgui.new_frame()
        w, h = self.imgui.io.display_size
        ...
        if (
            ...
            if self.select_object(x * C.ui_scale, y * C.ui_scale) or not isinstance(
        ...
        if imgui.begin_popup("Context Menu"):
            if self.scene.selected_object is None or not isinstance(
                ...
                self.scene.selected_object.gui_context_menu(
                    imgui, *self._context_menu_position
                )
            imgui.end_popup()
        self.imgui_user_interacting = False
        if self.render_gui:
    ...
    def gui_scene(self):
        ...
        imgui.set_next_window_size(
            self.imgui.io.display_size[0] * 0.25,
            self.imgui.io.display_size[1] * 0.7,
        ...
        if imgui.begin_main_menu_bar():
            ...
            if imgui.begin_menu("View", True):
                ...
                if imgui.begin_menu("Viewports"):
                    ...
                    imgui.end_menu()
                _, self.render_gui = imgui.menu_item(
                    "Render GUI", None, self.render_gui, True
                )
                imgui.end_menu()
            if imgui.begin_menu("Camera", True):
                ...
                is_ortho = (
                    False
                    if self.viewports[0].using_temp_camera
                    else self.scene.camera.is_ortho
                )
                ...
                if imgui.begin_menu(
                    "Control modes", enabled=not self.viewports[0].using_temp_camera
                ):

                    def mode(name, mode):
                        selected = imgui.menu_item(
                            name, None, self.scene.camera.control_mode == mode
                        )[1]
                        if selected:
                            self.reset_camera()
    ...
    def gui_export_video(self):
        ...
        if imgui.begin_popup_modal(
            ...
            if self.export_format != "webm":
                imgui.push_style_var(imgui.STYLE_ALPHA, 0.2)
                self.export_transparent = False
            _, self.export_transparent = imgui.checkbox(
                "Transparent background", self.export_transparent
            )
            if self.export_format != "webm":
                imgui.pop_style_var()
                self.export_transparent = False
            ...
            elif self.export_animation:
                imgui.text("Error: Animation range is empty")
            else:
                imgui.text("Error: Duration is 0 seconds")
            imgui.spacing()
            button_width = (
            ...
            if getattr(self, "_export_error", None):
                imgui.text_wrapped(self._export_error)
            if frames <= 0:
                imgui.pop_style_var()
            imgui.end_popup()
    ...
    def gui_playback(self):
        ...
        imgui.set_next_window_position(
            50, 100 + self.imgui.io.display_size[1] * 0.7, imgui.FIRST_USE_EVER
        )
        imgui.set_next_window_size(
            self.imgui.io.display_size[0] * 0.4,
            self.imgui.io.display_size[1] * 0.175,
            imgui.FIRST_USE_EVER,
        )
    ...
    def gui_shortcuts(self):
        if self._show_shortcuts_window:
            imgui.set_next_window_position(
                self.imgui.io.display_size[0] * 0.6, 200, imgui.FIRST_USE_EVER
            )
            imgui.set_next_window_size(
                self.imgui.io.display_size[0] * 0.35, 350, imgui.FIRST_USE_EVER
            )
    ...
    def gui_inspect(self):
        ...
        if self.selected_mode == "inspect":
            imgui.set_next_window_position(
                self.imgui.io.display_size[0] * 0.6, 50, imgui.FIRST_USE_EVER
            )
            imgui.set_next_window_size(
                self.imgui.io.display_size[0] * 0.35, 140, imgui.FIRST_USE_EVER
            )
    ...
    def mesh_mouse_intersection(self, x: int, y: int):
        ...
        x, y = self._mouse_to_buffer(x, y)
        point_world, obj_id, tri_id, instance_id = self.renderer.read_fragmap_at_pixel(
            x, y
        )
    ...
    def mouse_press_event(self, x: int, y: int, button: int):
        ...
        if not self.imgui_user_interacting:
            ...
            if button == self._left_mouse_button:
                ...
                self._moving_camera_viewport.camera.rotate_start(
                    x, y, *self._moving_camera_viewport.extents[2:]
                )
    ...
    def mouse_drag_event(self, x: int, y: int, dx: int, dy: int):
        ...
        if not self.imgui_user_interacting:
            ...
            if self._rotate_camera:
                self.reset_camera(self._moving_camera_viewport)
                x, y = self._mouse_to_viewport(x, y, self._moving_camera_viewport)
                self._moving_camera_viewport.camera.rotate(
                    x, y, dx, dy, *self._moving_camera_viewport.extents[2:]
                )
            if (
    ...
    def mouse_scroll_event(self, x_offset: float, y_offset: float):
        ...
        if not self.imgui_user_interacting:
            ...
            if v:
                self.reset_camera(v)
                v.camera.dolly_zoom(
                    np.sign(y_offset), self.wnd.modifiers.shift, self.wnd.modifiers.ctrl
                )
    ...
    def export_video(
        ...
        from osim_viewer._rendering.utils.video import export_video
    ...
    def on_files_dropped(self, x, y, paths):
