...
from osim_viewer._rendering.utils import set_lights_in_program, set_material_properties
...
from osim_viewer._rendering.utils.utils import compute_vertex_and_face_normals_sparse
...
class Meshes(Node):
    ...
    def current_transformed_vertices(self):
        return (
            self.current_vertices @ self.model_matrix[:3, :3].T
            + self.model_matrix[:3, 3]
        )
    ...
    def _upload_buffers(self):
        ...
        if self.face_colors is None:
            ...
            shape = (
                min(self.faces.shape[0], 8192),
                (self.faces.shape[0] + 8191) // 8192,
            )
            self.face_colors_texture.write(
                self.current_face_colors.astype("f4")
                .tobytes()
                .ljust(shape[0] * shape[1] * 16)
            )
        if self.has_texture:
    ...
    def _show_normals(self):
        ...
        from osim_viewer._rendering.renderables.arrows import Arrows
