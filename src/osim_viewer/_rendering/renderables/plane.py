...
class Plane(Node):
    ...
    def from_normal(
        ...
        **kwargs,
        ...
        if tangent is None:
            if abs(np.dot(normal, np.array([1.0, 0.0, 0.0]))) < 0.001:
...
class ChessboardPlane(Node):
    ...
    def gui(self, imgui):
        _, self.c1 = imgui.color_edit4(
            "Color 1##color{}'".format(self.unique_name), *self.c1
        )
        _, self.c2 = imgui.color_edit4(
            "Color 2##color{}'".format(self.unique_name), *self.c2
        )
