...
from osim_viewer._rendering.renderables.spheres import SpheresTrail
...
def _rotate_cylinder_to(target, vs, ns):
    ...
    theta = np.arccos(dot)
    is_col = np.linalg.norm(a, axis=1) < 1e-05
...
class Lines2D(Node):
    ...
    def __init__(self, lines, color=(0.0, 0.0, 1.0, 1.0), mode="line_strip", **kwargs):
        """
        Initializer.
        :param lines: Set of 3D coordinates as a np array of shape (F, L, 3) or (L, 3).
        :param color: Color of the line (4-tuple) or array of color (N_LINES, 4), one for each line.
    ...
    def make_renderable(self, ctx: moderngl.Context):
        ...
        self.vbo_vertices = ctx.buffer(
            self._get_vertices().astype("f4").tobytes(), dynamic=True
        )
        self.vbo_colors = ctx.buffer(
            self._get_colors().astype("f4").tobytes(), dynamic=True
        )
