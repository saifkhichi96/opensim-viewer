...
from osim_viewer._rendering.renderables.spheres import Spheres
from osim_viewer._rendering.scene.node import Node
from osim_viewer._rendering.utils.mocap import load_markers
...
class Markers(Node):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super(Markers, self).__init__(
            name, n_frames=points.shape[0], color=color, **kwargs
        )
        self.markers_labels = markers_labels
        self.marker_trajectory = points
        self.color = color
        self._z_up = z_up
        ...
        if self.marker_trajectory.shape[1] > 200:
            as_spheres = False
            print(
                f"Too many markers ({self.marker_trajectory.shape[1]}). Switching to pointcloud."
            )
    ...
    def from_c3d(
        ...
        **kwargs,
        ...
        markers_array, markers_labels, fps_in = load_markers(
            c3d_path, nb_markers_expected
        )
        print(
            f"fps_in={fps_in} fps_out={fps_out} markers_array.shape={markers_array.shape}"
        )
        if y_up:
            markers_array[:, :, [0, 1, 2]] = markers_array[:, :, [0, 2, 1]]
            markers_array[:, :, 2] = -markers_array[:, :, 2]
    ...
    def from_SSM_pkl(
        ...
        **kwargs,
        ...
        """Load a sequence from an npz file. The filename becomes the name of the sequence"""
        markers_data = pkl.load(open(ssm_pkl_path, "rb"), encoding="latin1")
        ...
        markers_array[:, :, [0, 1, 2]] = markers_array[:, :, [2, 1, 0]]
        markers_array[:, :, 0] = -markers_array[:, :, 0]
        if fps_out is not None and fps_in != fps_out:
            assert fps_in % fps_out == 0, "fps_out must be a interger divisor of fps_in"
            mask = np.arange(0, markers_array.shape[0], int(fps_in // fps_out))
            markers_array = markers_array[mask]
    ...
    def from_file(cls, mocap_file, **kwargs):
        ...
        if mocap_file.endswith(".npz"):
            return cls.from_synthetic(mocap_file, **kwargs)
        if mocap_file.endswith(".pkl"):
            return cls.from_SSM_pkl(mocap_file, **kwargs)
        raise ValueError(
            f"Unknown mocap file format: {mocap_file}, must be .c3d, .npz or .pkl"
        )
