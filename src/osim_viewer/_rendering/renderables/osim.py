...
from osim_viewer._rendering.utils import to_numpy as c2c
from osim_viewer._rendering.utils.colors import vertex_colors_from_weights

try:
    import nimblephysics as nimble
except ImportError:
    raise ImportError(
        "nimblephysics not found. Please install nimblephysics to use this module."
    )
...
def load_osim(osim_path, geometry_path=None, ignore_geometry=False):
    ...
    assert os.path.exists(
        osim_path
    ), f"Could not find osim file {os.path.abspath(osim_path)}"
    osim_path = os.path.abspath(osim_path)
    file_geometry_path = os.path.join(os.path.dirname(osim_path), "Geometry")
    geometry_path = geometry_path or file_geometry_path
    if not os.path.exists(file_geometry_path) or ignore_geometry:
        if ignore_geometry and os.path.exists(file_geometry_path):
            ...
            print(
                f"WARNING: No Geometry folder found at {file_geometry_path}, using {geometry_path} instead"
            )
...
class OSIMSequence(Node):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        body_nodes = [
            osim.skeleton.getBodyNode(i) for i in range(osim.skeleton.getNumBodyNodes())
        ]
        self.node_names = [n.getName() for n in body_nodes]
        self.meshes_dict = {}
        self.indices_dict = {}
        self.generate_meshes_dict()
        ...
        if self._show_joint_angles:
            ...
            self.rbs = RigidBodies(
                self.joints, global_oris, length=0.01, name="Joint Angles"
            )
        ...
        if color_markers_per_index:
            marker_index_colors = vertex_colors_from_weights(
                weights=range(len(self.marker_trajectory[0])),
                scale_to_range_1=True,
                alpha=1,
            )[np.newaxis, :, :]
        ...
        self.markers_seq = Markers(
            points=self.marker_trajectory,
            markers_labels=self.markers_labels,
            point_size=10.0,
            **kwargs,
        )
    ...
    def per_part_bone_colors(self):
        ...
        color_palette = vertex_colors_from_weights(
            np.arange(len(self.node_names)), shuffle=True
        )
    ...
    def per_part_marker_colors(self):
        colors = vertex_colors_from_weights(
            np.arange(len(self.node_names)), alpha=1, shuffle=True
        )
        rigging_file = None
        if self.osim_path is not None:
            rigging_file = self.osim_path.replace(".osim", "_rigging.pkl")
        if not rigging_file is None and os.path.exists(rigging_file):
            ...
            print(
                f"No rigging file {rigging_file} found. Fetching rigging for coloring."
            )
            colors = vertex_colors_from_weights(
                np.arange(len(self.node_names)), alpha=1, shuffle=True
            )
    ...
    def a_pose(cls, osim_path=None, **kwargs):
        ...
        if osim_path is None:
            raise ValueError("An explicit OpenSim model path is required.")
    ...
    def zero_pose(cls, osim_path=None, **kwargs):
        ...
        if osim_path is None:
            osim: nimble.biomechanics.OpenSimFile = (
                nimble.models.RajagopalHumanBodyModel()
            )
            osim_path = "RajagopalHumanBodyModel.osim"
        else:
    ...
    def from_files(
        ...
        **kwargs,
        ...
        print(
            f"Estimated fps for the .mot sequence: {fps_estimated}, rounded to {fps_in}"
        )
        if not ignore_fps:
            assert (
                abs(1 - fps_estimated / fps_in) < 1e-05
            ), f"FPS estimation might be bad, {fps_estimated} rounded to {fps_in}, check."
            if fps_out is not None:
                assert (
                    fps_in % fps_out == 0
                ), "fps_out must be a interger divisor of fps_in"
                mask = np.arange(0, motion.shape[0], fps_in // fps_out, dtype=int)
                print(
                    f"Resampling from {fps_in} to {fps_out} fps. Keeping every {fps_in // fps_out}th frame"
                )
                motion = motion[mask]
            del mot
        else:
            fps_out = fps_in
        return cls(
            osim, motion, osim_path=osim_path, fps=fps_out, fps_in=fps_in, **kwargs
        )
    ...
    def fk(self):
        ...
        verts = np.zeros(
            (
                self.n_frames,
                self.template.vertices.shape[0],
                self.template.vertices.shape[1],
            )
        )
        ...
        for frame_id in (pbar := tqdm.tqdm(range(self.n_frames))):
            ...
            markers[frame_id, :, :] = np.vstack(
                list(
                    self.osim.skeleton.getMarkerMapWorldPositions(
            ...
            for ni, node_name in enumerate(self.node_names):
                ...
                if mesh is not None:
                    ...
                    transfo = self.osim.skeleton.getBodyNode(
                        node_name
                    ).getWorldTransform()
                    part_verts = np.concatenate(
                        [part_verts, np.ones((mesh.vertices.shape[0], 1))], axis=1
                    )
